import type { CommanderDashboardData } from "./commander-hq-types";

export const mockCommanderDashboard: CommanderDashboardData = {
  identity: {
    twitter: {
      id: "mock-x-001",
      username: "scream_vision",
      displayName: "Scream Vision",
    },
    linkedWallets: [
      {
        address: "E2UmXw9kZdJmBpZVtShKwrsXeT9X31qsiu8GKwaSvmMb",
        chain: "solana",
        isPrimary: true,
      },
    ],
  },
  commander: {
    displayName: "Scream Vision",
    username: "scream_vision",
    rank: "Alpha Officer",
    rankImage: "/ranks/web/10-alpha-officer.png",
    armySize: 42,
    commanderScore: 9320,
    memberSince: "21 Jul 2026",
    nextRank: "Alpha Commander",
    soldiersNeeded: 13,
    rankProgress: 13,
    primarySoldier: {
      mint: "mock-gas-001",
      name: "GROYPER #1",
      image: "/nfts/images/nft_1.png",
      rarity: "#427",
      rank: "Initiate",
      traits: [
        { traitType: "Eyes", value: "Shiner Eyes" },
        { traitType: "Hats", value: "M1 Helmet" },
      ],
    },
  },
  soldiers: [
    {
      mint: "mock-gas-001",
      name: "GROYPER #1",
      image: "/nfts/images/nft_1.png",
      rarity: "#427",
      rank: "Initiate",
      traits: [{ traitType: "Hats", value: "M1 Helmet" }, { traitType: "Eyes", value: "Shiner Eyes" }],
    },
    {
      mint: "mock-gas-004",
      name: "GROYPER #4",
      image: "/nfts/images/nft_4.png",
      rarity: "#188",
      rank: "Operative",
      traits: [{ traitType: "Clothes", value: "Field Uniform" }, { traitType: "Hands", value: "Based Hands" }],
    },
    {
      mint: "mock-gas-008",
      name: "GROYPER #8",
      image: "/nfts/images/nft_8.png",
      rarity: "#301",
      rank: "Vanguard",
      traits: [{ traitType: "Background", value: "Night Ops" }, { traitType: "Eyes", value: "Focused" }],
    },
    {
      mint: "mock-gas-012",
      name: "GROYPER #12",
      image: "/nfts/images/nft_12.png",
      rarity: "#094",
      rank: "Strategist",
      traits: [{ traitType: "Hats", value: "Command Beret" }, { traitType: "Body", value: "Based Body" }],
    },
    {
      mint: "mock-gas-024",
      name: "GROYPER #24",
      image: "/nfts/images/nft_24.png",
      rarity: "#221",
      rank: "Tactical Lead",
      traits: [{ traitType: "Clothes", value: "Recon Gear" }, { traitType: "Mouth", value: "Smirk" }],
    },
    {
      mint: "mock-gas-036",
      name: "GROYPER #36",
      image: "/nfts/images/nft_36.png",
      rarity: "#057",
      rank: "Alpha Officer",
      traits: [{ traitType: "Background", value: "Command Room" }, { traitType: "Hats", value: "Officer Cap" }],
    },
  ],
  treasury: {
    totalValueUsd: 42930.71,
    change24hUsd: 8230.45,
    change24hPercent: 8.23,
    investedUsd: 22608.26,
    realizedPnlUsd: 12430.12,
    unrealizedPnlUsd: 7892.33,
  },
  provisions: {
    tokenBalance: 143221,
    tokenPriceUsd: 0.02941,
    tokenValueUsd: 4210.15,
    averageEntryUsd: 0.01723,
    change24hPercent: 12.45,
  },
  market: {
    tokenPriceUsd: 0.02941,
    change24hPercent: 12.45,
    marketCapUsd: 29410000,
    liquidityUsd: 1640000,
    volume24hUsd: 883500,
    holders: 11842,
    chart: {
      "1H": [2.61, 2.64, 2.62, 2.68, 2.71, 2.69, 2.76, 2.81, 2.86, 2.94],
      "24H": [2.19, 2.24, 2.21, 2.32, 2.29, 2.41, 2.38, 2.55, 2.61, 2.58, 2.73, 2.94],
      "7D": [1.72, 1.89, 1.84, 2.02, 1.96, 2.17, 2.11, 2.34, 2.29, 2.63, 2.52, 2.94],
      "30D": [0.91, 1.12, 1.04, 1.28, 1.54, 1.47, 1.83, 1.69, 2.06, 2.31, 2.18, 2.94],
    },
  },
  medals: [
    { id: "first-wave", name: "First Wave", code: "01", unlocked: true },
    { id: "army-builder", name: "Army Builder", code: "10", unlocked: true },
    { id: "diamond-hands", name: "Diamond Hands", code: "30", unlocked: true },
    { id: "signal-corps", name: "Signal Corps", code: "X", unlocked: true },
    { id: "high-command", name: "High Command", code: "HC", unlocked: false },
  ],
  achievements: [
    { id: "early", name: "Early Recruit", description: "Joined among the first 100 commanders", state: "unlocked" },
    { id: "diamond", name: "Diamond Hands", description: "Held $GROYPER for 30 days", state: "unlocked" },
    { id: "builder", name: "Army Builder", description: "Own 10+ GAS soldiers", state: "progress", progress: 82 },
    { id: "supreme", name: "Supreme Command", description: "Reach the highest collection rank", state: "locked" },
    { id: "classified", name: "Classified Operation", description: "Hidden achievement", state: "hidden" },
  ],
  dataMode: "mock",
};

